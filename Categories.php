...
function wdcat_save_cat()
{
	...
	$save_or_no= $wpdb->insert(
		$wpdb->prefix.'spidercatalog_product_categories', array(
			'id'	=> NULL,
	...
	if(isset($_POST["parent"]) && $_POST["parent"]!=0){
		foreach($rows1 as $par_cat){
			$wpdb->update($wpdb->prefix.'spidercatalog_product_categories', array( 'param'    =>$par_cat, ), 
			array('id'=>$id),
			array(  '%s' )
			);
		}
...
function wdcat_change_cat( $id ){
	...
	$savedd=$wpdb->update(
		$wpdb->prefix.'spidercatalog_product_categories', array('published'    =>$published,), 
		array('id'=>$id),
		array(  '%d' )
	);
	?>
...
function wdcat_apply_cat($id) {	
	...
	if($_POST["parent"]!=0){
		...
		if(isset($_POST["parent"]) && $_POST["parent"]!=$id_bef){
			foreach($rows1 as $par_cat){
				$wpdb->update(
					$wpdb->prefix.'spidercatalog_product_categories', array('param'    =>$par_cat,), 
					array('id'=>$id),
					array(  '%s' )
				);
			}

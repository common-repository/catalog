		...
		case 'apply':
            if ($id) {
                ...
                if ($true) {
					...
					<h1>Database Error Please install plugin again</h1>
					<?php wdcat_showCategory();
                }
            }
            break;
        ...
        <form method="post" action="<?php echo admin_url('admin.php?page=Uninstall_Spider_Catalog'); ?>">
			...
			<div class="wrap">
				...
				<p style="text-align: center;">
					...
					<br/><br/>
					<input type="submit" name="do" value="<?php echo 'UNINSTALL Spider Catalog'; ?>" class="button-primary" onClick="return confirm('<?php echo 'You Are About To Uninstall Spider Catalog From WordPress.\nThis Action Is Not Reversible.\n\n Choose [Cancel] To Stop, [OK] To Uninstall.'; ?>')"/>
				</p>
			</div>
        </form>
    <?php
		...
		<body id="link" onLoad="tinyMCEPopup.executeOnLoad('init();');document.body.style.display='';" style="" dir="ltr" class="forceColors">
			<form name="spider_cat" action="#" method="post">
				...
				<div class="panel_wrapper">
					<div id="Single_product_panel" class="panel current">
						...
						<tbody>
							<tr>
								...
								<td>
									<select name="spider_catalog_product" id="spider_catalog_product">
										<option value="- Select a Product -" selected="selected">- Select a Product -</option>
										<?php foreach ($single_products as $single_product) { ?>
					...
					<div id="Products_list_panel" class="panel nogut">
						<table border="0" cellspacing="0" style="font-size: 11px !important;">
						<tbody>
							<tr>
								<td nowrap="nowrap"><label for="Spider_cat_Category" class="optTitle">Select Category</label></td>
								<td>
									<select name="Spider_cat_Category" id="Spider_cat_Category">
							...
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Show category details</label></td>
								<td>
									<input type="radio" name="cat_show" id="paramsshow_category_details1" value="1" checked="checked">
										<label for="paramsshow_category_details1">Show</label>
									<input type="radio" name="cat_show" id="paramsshow_category_details0" value="0">
										<label for="paramsshow_category_details0">Hide</label>
								</td>
							</tr>
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Product display type</label></td>
								<td>
							...
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Show Subcategories</label></td>
								<td>
									<input type="radio" name="show_sub" id="show_subategory1" value="1" checked="checked">
										<label for="show_subategory1">Yes</label>
									<input type="radio" name="show_sub" style="margin-left: 12px;" id="show_subcategory0" value="0">
										<label for="show_subcategory0">No</label>
								</td>
							</tr>
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Show Subcategory Products</label></td>
								<td>
									<input type="radio" name="show_sub_prod" id="show_sub_products1" value="1">
									...
									<input type="radio" name="show_sub_prod" id="show_sub_products2" style="margin-left: 12px;" value="2" checked="checked">
										<label for="show_sub_products2">Chosen</label>
								</td>
							</tr>
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Show Products</label></td>
								<td>
									<input type="radio" name="show_prod" id="show_products1" value="1" checked="checked">
										<label for="show_products1">Yes</label>
									<input type="radio" name="show_prod" style="margin-left: 12px;" id="show_products0" value="0">
										<label for="show_products0">No</label>
								</td>
							</tr>
						</tbody>
						</table>
...
function catalog_Featured_Plugins() {
  ...
  <div id="main_featured_plugins_page_catalog">
    ...
    <ul id="featured-plugins-list">
      <?php
      foreach ($plugins as $key_sp => $plugins) {
        if ($current_plugin != $key_sp) {
          ?>
      <li class="<?php echo $key_sp; ?>">
        <div class="product">
          <div class="title">
...
add_filter('tw_get_block_editor_assets', 'spider_catalog_register_block_editor_assets');
add_action( 'enqueue_block_editor_assets', 'spider_catalog_enqueue_block_editor_assets');
add_action('wp_ajax_spidercatalogswindow_new', 'spidercatalogswindow_new');

function spider_catalog_register_block_editor_assets($assets) {
    $version = '1.7.3';
...
function spider_catalog_enqueue_block_editor_assets() {
    $key_sp = 'tw/spider-catalog';
    $key_sp_categories = 'tw/spider-catalog-categories';
    
    $plugin_name = 'Spider Catalog';
	$plugin_name_categories = __('Spider Catalog Categories', 'spider_catalog');
	
	$icon_url = plugin_dir_url(__FILE__). '/tw-gb/spider_catalog.svg';
	$icon_svg = plugin_dir_url(__FILE__). '/tw-gb/spider_catalog_grey.svg';
	
    $url = add_query_arg(array('action' => 'spidercatalogswindow_new'), admin_url('admin-ajax.php'));
	$data = sp_get_shortcode_data();
    ?>
    <script>
	if ( !window['tw_gb'] ) {
        window['tw_gb'] = {};
	}
	if ( !window['tw_gb']['<?php echo $key_sp; ?>'] ) {
        window['tw_gb']['<?php echo $key_sp; ?>'] = {
          title: '<?php echo $plugin_name; ?>',
          titleSelect: '<?php echo sprintf(__('Select %s', 'spider_catalog'), $plugin_name); ?>',
	...
	if ( !window['tw_gb']['<?php echo $key_sp_categories; ?>'] ) {
        window['tw_gb']['<?php echo $key_sp_categories; ?>'] = {
    ...
    foreach ($wp_scripts->registered as $key_sp => $value) {
      // Check for an older versions with prefix.
      if (strpos($key_sp, 'tw-gb-block') > 0) {
        wp_deregister_script( $key_sp );
        wp_deregister_style( $key_sp );
      }
    }
    ...
    wp_localize_script('tw-gb-block', 'tw_obj', array(
      'nothing_selected' => __('Nothing selected.', 'spider_contacts'),
      'empty_item' => __('- Select -', 'spider_contacts'),
    ));
    wp_enqueue_style('tw-gb-block', $assets['css_path'], array( 'wp-edit-blocks' ), $assets['version']);
}
 
function sp_get_shortcode_data() {
	global $wpdb;
	$rows = $wpdb->get_results("SELECT `id`, `name` as name FROM `" . $wpdb->prefix . "spidercatalog_products`");
	$data = array();
	$data['shortcode_prefix'] = 'Spider_Catalog_Product';
	$data['inputs'][] = array(
		'type' => 'select',
		'id' => 'spider_catalog_cat_id',
		'name' => 'spider_catalog_cat_id',
...
function spidercatalogswindow_new(){
	global $wpdb;
	function wdcat_open_cat_in_tree($catt, $tree_problem = '', $hihiih = 1)
	{
		global $wpdb;
		...
		foreach ($catt as $local_cat) {
			$local_cat->name = $tree_problem . $local_cat->name;
			array_push($trr_cat, $local_cat);
			$new_cat_query = "SELECT  a.* ,  COUNT(b.id) AS count, g.par_name AS par_name FROM " . $wpdb->prefix . "spidercatalog_product_categories  AS a LEFT JOIN " . $wpdb->prefix . "spidercatalog_product_categories AS b ON a.id = b.parent LEFT JOIN (SELECT  " . $wpdb->prefix . "spidercatalog_product_categories.ordering as ordering," . $wpdb->prefix . "spidercatalog_product_categories.id AS id, COUNT( " . $wpdb->prefix . "spidercatalog_products.category_id ) AS prod_count FROM " . $wpdb->prefix . "spidercatalog_products, " . $wpdb->prefix . "spidercatalog_product_categories WHERE " . $wpdb->prefix . "spidercatalog_products.category_id = " . $wpdb->prefix . "spidercatalog_product_categories.id GROUP BY " . $wpdb->prefix . "spidercatalog_products.category_id) AS c ON c.id = a.id LEFT JOIN (SELECT " . $wpdb->prefix . "spidercatalog_product_categories.name AS par_name," . $wpdb->prefix . "spidercatalog_product_categories.id FROM " . $wpdb->prefix . "spidercatalog_product_categories) AS g ON a.parent=g.id WHERE a.name LIKE '%" . esc_html($search_tag) . "%' AND a.parent=" . $local_cat->id . " group by a.id";
			$new_cat = $wpdb->get_results($new_cat_query);
			wdcat_open_cat_in_tree($new_cat, $tree_problem . "— ", 0);
		}
		return $trr_cat;
	...
	<html xmlns="http://www.w3.org/1999/xhtml">
		<body>	
			<form name="spider_cat" action="#" method="post" class="gut">
				<div class="tabs" role="tablist" tabindex="-1">
					...
					<div id="Products_list_panel" class="panel">
						...
						<tbody>
							...
							<tr>
								<td nowrap="nowrap" valign="top"><label for="showtype" class="optTitle">Product display type</label></td>
								<td id="prodViews">
									<div><input type="radio" name="cat_list" id="paramslist_category_type0" value="" checked="checked" class="paramslist_category">
									<label for="paramslist_category_type0">Cells 1</label></div>
									<div ><input type="radio" name="cat_list" id="paramslist_category_type1" value="list"class="paramslist_category">
									<label for="paramslist_category_type1">List</label></div>
									<div ><input type="radio" name="cat_list" id="paramslist_category_type2" value="cells2" class="paramslist_category">
									<label for="paramslist_category_type2">Cells 2</label></div>
									<div ><input type="radio" name="cat_list" id="paramslist_category_type3" value="wcells" class="paramslist_category">
									<label for="paramslist_category_type3">Wide Cells</label></div>
									<div ><input type="radio" name="cat_list" id="paramslist_category_type4" value="thumb" class="paramslist_category">
									<label for="paramslist_category_type4">Thumbnails</label></div>
									<div ><input type="radio" name="cat_list" id="paramslist_category_type5" value="cells3" class="paramslist_category">
									<label for="paramslist_category_type5">Cells 3</label></div>
								</td>
							</tr>
							<tr>
			...
			<style>			
			.gut #Products_list_panel table label.optTitle { 
				font-size:16px;
				font-style:italic;
			}
			.gut #Spider_cat_Category { padding:8px; }
			...
			.gut #insert:hover { background-color: #329838; }
			.gut #insert:focus { outline:none; }
			</style>
			<script type="text/javascript">
			function insert_spider_catalog() {
				var lists;
				...
				if (window.parent.window['wdg_cb_tw/spider-catalog-categories']) {
					window.parent['wdg_cb_tw/spider-catalog-categories'](tagtext, 0);
					return;
				} else {
